...
#SBATCH --job-name=bf_lds_fds_sqinv_aip   # Job name
...
echo "Starting BF LDS FDS sqrt_inv training at date $(date)"
...
# --- Configuration based on run_bf_lds_fds_sqinv_chan.sh ---
...
if [ $? -eq 0 ]; then
    echo "Successfully completed BF LDS FDS sqrt_inv training"
else
    echo "Error in BF LDS FDS sqrt_inv training"
    exit 1
fi
...
# Usage:
# sbatch aip_scripts/run_bf_lds_fds_sqinv_aip.sh 